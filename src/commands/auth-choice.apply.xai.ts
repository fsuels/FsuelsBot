...
export async function applyAuthChoiceXAI(
  ...
  if (optsKey) {
    setXaiApiKey(normalizeApiKeyInput(optsKey), params.agentDir, params.writePlan);
    hasCredential = true;
  }
  ...
  if (!hasCredential) {
    ...
    if (envKey) {
      ...
      if (useExisting) {
        setXaiApiKey(envKey.apiKey, params.agentDir, params.writePlan);
        hasCredential = true;
      }
    }
  }
  ...
  if (!hasCredential) {
    ...
    setXaiApiKey(normalizeApiKeyInput(String(key)), params.agentDir, params.writePlan);
  }
