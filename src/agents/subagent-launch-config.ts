...
function normalizeInheritValue(value: string | undefined): string | "inherit" | undefined {
  if (!value) {
    return undefined;
  }
...
export function resolveSubagentLaunchConfig(params: {
  ...
  const modelCandidates: Array<{
    value: string | "inherit" | undefined;
  ...
  for (const candidate of thinkingCandidates) {
    ...
    if (!normalized) {
      return {
        ok: false,
        error: `Invalid thinking level "${candidate.value}". Use one of: ${formatThinkingLevels(requesterModel.provider, requesterModel.model)}.`,
      };
    }
