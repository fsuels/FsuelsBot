...
describe("sessions_spawn structured task specs", () => {
  ...
  it("passes structured-output settings through to the child agent request", async () => {
    callGatewayMock.mockImplementation(async (opts: unknown) => {
      const request = opts as { method?: string; params?: Record<string, unknown> };
      if (request.method === "agent") {
        return {
          runId: String(request.params?.idempotencyKey ?? "run-structured"),
          status: "accepted",
        };
      }
      if (request.method === "agent.wait") {
    ...
    const result = await tool.execute("structured-output-task", {
      ...
      structuredOutputName: "verification_report",
    });
    ...
    const agentCall = callGatewayMock.mock.calls.find(
      ([arg]) => (arg as { method?: string }).method === "agent",
    )?.[0] as { params?: { structuredOutputSchema?: unknown; structuredOutputName?: unknown } };
    expect(agentCall.params?.structuredOutputSchema).toMatchObject({
      type: "object",
      required: ["ok"],
    });
    expect(agentCall.params?.structuredOutputName).toBe("verification_report");
  });
});
