...
- `/revenue ...` (owner-only revenue operations: checklist, daily, autopilot, paid logging, and reports)
...
- `/task show|list|set|active|paused|completed|archived|done` (aliases: `/tasks`, `/resume`, `/switch`, `/newtask`, `/link`, `/archive`, `/close`; manage task context and lifecycle)
- `/pin list|remove|fact|preference|constraint|temporary ...` (aliases: `/pins`, `/unpin`; typed durable pins; supports `--task`, `--ttl`, `--entity`)
- `/autoswitch on|off|status` (toggle inferred task auto-switching)
- `/mode minimal|supportive|status` (set memory guidance mode)
- `/forget <text>|--task <id>|--entity <name>|--before <date>` (explicit forgetting for memory files and pins)
