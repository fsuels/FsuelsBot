---
updated: 2026-01-29
---

# Phase 2 Product Prep - Valentine Import Batch

**Created:** 2026-01-29 23:35 EST
**Status:** Ready for pricing/SEO completion after import
**Deadline:** 12 days until Feb 10 order cutoff

---

## 7 Products Overview

| # | 1688 ID | Status | BuckyDrop | Pricing | SEO |
|---|---------|--------|-----------|---------|-----|
| 1 | 853944666153 | âœ… Added | âœ… Product List | âœ… Done | â³ Pending |
| 2 | 969885791145 | â³ Pending | âŒ | âŒ | âŒ |
| 3 | 934291597297 | â³ Pending | âŒ | âŒ | âŒ |
| 4 | 979076019926 | â³ Pending | âŒ | âŒ | âŒ |
| 5 | 862560576658 | â³ Pending | âŒ | âŒ | âŒ |
| 6 | 980618496327 | â³ Pending | âŒ | âŒ | âŒ |
| 7 | 971972174266 | â³ Pending | âŒ | âŒ | âŒ |

---

## Product 1: Wool Lamb Family Matching Outfit (853944666153)

**1688 URL:** https://detail.1688.com/offer/853944666153.html

### Pricing Calculation âœ…
| Component | CNY | USD |
|-----------|-----|-----|
| Product price | Â¥63-99 | $8.86-13.92 |
| Domestic Shipping | Â¥5 | $0.70 |
| Platform Fee | Â¥5.5 | $0.77 |
| Value-Added Services | Â¥13.1 | $1.84 |
| **Base Total** | ~Â¥86.6 | ~$12.18 |
| Int'l Shipping (est.) | ~Â¥50 | ~$7.00 |
| **TOTAL COST** | ~Â¥136 | ~$19.18 |

**Minimum Price (Ã—1.5):** $28.77 â†’ **$29.99**
**Suggested Price:** $34.99-39.99 (pending competitor check)

âš ï¸ **Note:** Min order is 2 units

### SEO Title (Draft)
`Wool Lamb Matching Family Sweatshirt - Mommy and Me | DLM`
(56 chars âœ…)

### SEO Description (Draft)
```
Match in cozy style with this **Wool Lamb Family Matching Sweatshirt**! Perfect for Valentine's Day photos and chilly weather cuddles.

**Why You'll Love It:**
- ðŸ’• Soft wool blend fabric - cozy and warm
- ðŸ’• Adorable lamb design kids love
- ðŸ’• Family sizes: Mom S-XXL, Kids 2T-10Y, Dad M-3XL
- ðŸ’• Machine washable, easy care

**Perfect for:**
- Valentine's Day matching photos ðŸ’
- Winter family outings
- Cozy at-home moments
- Gift for new moms

*Order by Feb 10 for Valentine's Day delivery!*
```

### Tags
`matching-sweatshirts`, `family-matching`, `valentines`, `winter`, `wool`, `mommy-and-me`, `lamb`

---

## Pricing Template (Products 2-7)

Fill in after BuckyDrop import:

```
### Product [#]: [Name] (ID)

**1688 URL:** https://detail.1688.com/offer/[ID].html

#### Pricing
| Component | CNY | USD |
|-----------|-----|-----|
| Product price | Â¥ | $ |
| Domestic Shipping | Â¥ | $ |
| Platform Fee | Â¥5.5 | $0.77 |
| Value-Added | Â¥ | $ |
| Int'l Shipping | Â¥ | $ |
| **TOTAL COST** | Â¥ | $ |

Min Price (Ã—1.5): $____
Suggested Price: $____

#### Competitor Check
- Amazon similar: $____ - $____
- Etsy similar: $____ - $____
- Decision: $_____

#### SEO Title
`[Product Name] - Mommy and Me [Type] | DLM`

#### Tags
```

---

## SEO Title Templates

For Valentine products, use these patterns:

| Product Type | Template | Example |
|--------------|----------|---------|
| Sweatshirt/Top | `[Feature] Mommy and Me Sweatshirt - Valentine's | DLM` | `Heart Print Mommy and Me Sweatshirt - Valentine's | DLM` |
| Pajamas | `Valentine's [Theme] Matching Family Pajamas | DLM` | `Valentine's Heart Matching Family Pajamas | DLM` |
| Dress | `[Style] Mommy and Me Dress - Valentine's Day | DLM` | `Red Heart Mommy and Me Dress - Valentine's Day | DLM` |
| Set | `[Theme] Matching Family Outfit Set | DLM` | `Valentine Heart Matching Family Outfit Set | DLM` |

**Rules:**
- Max 60 characters
- Include "Mommy and Me" or "Matching Family"
- End with "| DLM" (brand)
- Include seasonal keyword when relevant

---

## Competitor Pricing Benchmarks (Jan 2026)

**Source:** Walmart Valentine's Mommy & Me Collection

| Product Type | Price Range | Notes |
|--------------|-------------|-------|
| T-Shirt sets | $31.99 | Tstars brand, includes mom + kid |
| Budget sweatshirts | $11.21 - $13.98 | Musuos, Sunisery brands |
| Heart sweaters | $21.98+ | Jollidays, higher quality |
| Pajama sets | $15.99+ | 2-piece satin |
| Matching shirts | $16.95+ | Olive Loves Apple |

**Competitive Positioning for DLM:**
- **Budget items ($11-15):** Match or undercut by $1-2
- **Mid-range ($16-25):** Price at $24.99-29.99 with free shipping angle
- **Premium ($26-35):** Price at $29.99-39.99 â€” emphasize quality + free shipping

**Our advantage:** Free shipping included. Most Walmart items show "From $X" + shipping.

---

## Valentine Keywords to Use

**Primary:**
- mommy and me Valentine outfit
- matching Valentine's Day outfits
- Valentine mother daughter matching

**Secondary:**
- heart matching family pajamas
- Valentine mommy and me dress
- Valentine's Day matching set
- mother daughter Valentine outfit

**Long-tail:**
- mommy and me Valentine photo outfit
- matching family Valentine's Day shirts
- Valentine's Day twinning outfits for mom and daughter

---

## Description Urgency Copy

Add to ALL Valentine product descriptions:

```
â° **Valentine's Deadline:** Order by Feb 10 for guaranteed Valentine's Day delivery!
ðŸ’ Free shipping on all orders.
```

---

## Next Steps

1. **Francisco imports remaining 6 URLs to BuckyDrop**
2. **I complete pricing calculations** (need BuckyDrop cost breakdown)
3. **I draft SEO titles/descriptions** for each product
4. **Francisco pushes all 7 to Shopify as drafts**
5. **I polish listings in Shopify**
6. **Francisco sets Active**

---

*This document prepared while Council session runs. Ready for immediate use once imports complete.*

