...
## Key Rules
- Use `curl.exe` not `curl` — PowerShell aliases `curl` to `Invoke-WebRequest` [verified: 2026-01-26]
...
- ClawdHub search frequently times out — use broad queries [verified: 2026-01-27]
- `trash` command may not be available — check before using over `rm` [verified: 2026-01-26]
