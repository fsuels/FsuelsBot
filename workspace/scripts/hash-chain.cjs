...
function initHashChain(filePath) {
  ...
  const genesisEvent = {
    ...
    priority: "P0",
    content:
      "Hash chain initialized. All subsequent events are cryptographically linked. Tampering with any event will break the chain and be detectable.",
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!chainStarted) {
      if (event.type !== "chain_init") {
        errors.push({
          line: i + 1,
          error: "First hashed event must be chain_init",
          eventId: event.id,
        });
      }
      if (event.prevHash !== "0000000000000000") {
        errors.push({
          line: i + 1,
          error: "chain_init must have genesis prevHash (16 zeroes)",
          eventId: event.id,
        });
      }
      chainStarted = true;
